...
- **Proxmox credentials** in the migrator's cloud config, one of:
  ...
  - **A scoped API token** with `--token-copy-endpoint`. Install the [`pve-csi-copy`](../hack/pve-token-copy/) package on every Proxmox node and pass `--token-copy-endpoint` to the controller: the copy then runs through a permission-gated sibling endpoint (`Datastore.Audit` on the source + `Datastore.AllocateSpace` on the target), so no `root@pam` credential is needed. The token must also carry the VM/disk privileges the helper-VM conversion path uses (`VM.Allocate`, `VM.Config.Disk`, `Datastore.Allocate`). See [hack/pve-token-copy/README.md](../hack/pve-token-copy/README.md).
- Nodes labelled with `topology.kubernetes.io/region` / `zone` (or the `topology.proxmox.sinextra.dev/*` equivalents), and VM IDs resolvable from `Node.spec.providerID` (`proxmox://<region>/<vmid>`) or the `proxmox.sinextra.dev/instance-id` annotation.
