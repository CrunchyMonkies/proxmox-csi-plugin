...
- **Proxmox root credentials** (`root@pam` username/password) in the migrator's cloud config. The disk copy, conversion, and helper-VM APIs require root; the CSI controller's API token is not sufficient. Keep these in a **dedicated secret**, separate from the CSI controller's token config, so the least-privileged component never holds root.
